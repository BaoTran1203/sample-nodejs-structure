...
export class BaseResponse {
    ...
    static success(req: Request, res: Response, data: any, total: number = 0) {
        ...
        if (typeof data === 'string' || data instanceof String) {
            return res.status(200).json({ status: false, code: 200, msg: data }).end();
        }

        if (req.method === 'GET') {
            let msg = 'Lấy dữ liệu thành công.';
            if (total > 0) {
                return res.status(200).json({ status: false, code: 200, msg: msg, data: data, total: total }).end();
            }
            return res.status(200).json({ status: false, code: 200, msg: msg, data: data }).end();
        }
    ...
    static error(req: Request, res: Response, error: any | string) {
        ...
        if (error.name === 'JsonWebTokenError') {
            ...
            return res.status(401).json({ status: false, code: 401, name: error.name, msg: msg }).end();
        }
        ...
        if (error.name === 'NoContent') {
            ...
            return res.status(404).json({ status: false, code: 404, name: error.name, msg: msg }).end();
        }
